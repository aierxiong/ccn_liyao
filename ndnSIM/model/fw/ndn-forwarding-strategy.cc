                                ...
                                Ptr<Interest> interest)
{
  ...
  //20160113 start 
  NS_LOG_INFO("ForwardingStrategy interest Node:" << inFace ->GetNode()->GetId());
  NS_LOG_INFO("ForwardingStrategy interest Scope:" << interest->GetScope());

  std::cout << " node " << inFace ->GetNode() ->GetId()<< " interest name " << interest->GetName() <<std::endl;
  ...
  if (contentObject != 0)
    {
      ...
      if (interest->GetPayload ()->PeekPacketTag (maxHopTag))
        {
          contentObject->GetPayload ()->AddPacketTag (maxHopTag);
        }
       //20160115 maxhop end

        //20160113 start
        FwMaxBetwTag maxBetwTag;
        if (interest->GetPayload ()->PeekPacketTag (maxBetwTag))
        {
          contentObject->GetPayload ()->AddPacketTag (maxBetwTag);
        }
        FwMinBetwTag minBetwTag;
        if (interest->GetPayload ()->PeekPacketTag (minBetwTag))
        {
                            ...
                            Ptr<Data> data)
{
  ...
  if (pitEntry == 0)
  ...
  else
    {
      ...
      double maxBetw = 0;
      double nodeBetw = inFace ->GetNode()->GetBetw();
      std::cout << " node betw " << nodeBetw <<std::endl;
      if(data->GetPayload()->PeekPacketTag(maxBetwTag))
      {
        maxBetw = maxBetwTag.GetMax();
        std::cout<<" node "<< inFace ->GetNode()->GetId()<<" maxBetw "<< maxBetw <<std::endl;
      }
       FwMinBetwTag minBetwTag;
      double minBetw = 0;
      // double nodeBetw = inFace ->GetNode()->GetBetw();
      // std::cout << " node betw " << nodeBetw <<std::endl;
      if(data->GetPayload()->PeekPacketTag(minBetwTag))
      {
      ...
      FwMaxHopTag maxHopTag;
      uint32_t m_maxHop = 0;
      if (data->GetPayload ()->PeekPacketTag (maxHopTag))
        {
          m_maxHop = maxHopTag.GetMax();
          std::cout<<" node "<< inFace ->GetNode()->GetId()<<"  maxhop  "<< m_maxHop <<std::endl;
        }
        //20160115 maxhop end
