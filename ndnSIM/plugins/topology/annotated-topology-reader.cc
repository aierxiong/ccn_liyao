...
//edit start
// Ptr<Node>
// AnnotatedTopologyReader::CreateNode (const std::string name, double posX, double posY, uint32_t systemId, double preference)
// {
//   NS_LOG_FUNCTION (this << name << posX << posY << preference);
//   m_requiredPartitions = std::max (m_requiredPartitions, systemId + 1);

//   Ptr<Node> node = CreateObject<Node> (systemId, preference);
...
//20160113 start 
Ptr<Node>
AnnotatedTopologyReader::CreateNode (const std::string name, double posX, double posY, uint32_t systemId,  double Betw)
{
  NS_LOG_FUNCTION (this << name << posX << posY<<systemId << Betw);
  m_requiredPartitions = std::max (m_requiredPartitions, systemId + 1);
  ...
  while (!topgen.eof ())
    {
      ...
      //   }
      // zhu shi end
      lineBuffer >> name >> city >> latitude >> longitude >> systemId >>Betw;
      if (name.empty ()) continue;

      Ptr<Node> node;

      if (abs(latitude) > 0.001 && abs(latitude) > 0.001)
        node = CreateNode (name, m_scale*longitude, -m_scale*latitude, systemId, Betw);
      else
        {
          UniformVariable var (0,200);
          node = CreateNode (name, var.GetValue (), var.GetValue (), systemId, Betw);
          // node = CreateNode (name, systemId);
        }
    }
